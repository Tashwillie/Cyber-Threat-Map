...
export function generateMockAttack() {
    ...
    if (Math.random() < 0.8) {
        ...
        const possibleTargets = continentalCities.filter(c => c.region !== sourceCity.region &&
        ...
        if (possibleTargets.length > 0) {
            ...
            const fallbackTargets = continentalCities.filter(c => c.name !== sourceCity.name &&
                c.country !== sourceCity.country);
    ...
    if (targetCity.country === sourceCity.country) {
        const differentCountryTargets = continentalCities.filter(c => c.country !== sourceCity.country);
...
export function generateMockAttacks(count) {
